...
/// Convert the string that is SCREAMING_SNAKE_CASE to UpperCamelCase, to conform to rust
/// naming rules about enum variants.
...
/// Wrap urls found in the string with < .. > enclosing brackets to conform to rust doc rules
fn wrap_urls(text: &str) -> String {
    let url_regex = Regex::new(r"(https?://[^\s]+)").unwrap();
    url_regex.replace_all(text, "<$1>").to_string()
}
...
fn main() {
    ...
    writeln!(tags_file, "//! All labjack tag constants.").unwrap();
    writeln!(
        tags_file,
        "// This file is autogenerated, do not modify!. See `generate_tags.rs`"
    )
    .unwrap();
    writeln!(tags_file, "use crate::prelude::*;").unwrap();
    writeln!(tags_file, "use bytes::Bytes;").unwrap();
    writeln!(tags_file, "").unwrap();
    writeln!(errors_file, "//! All labjack error codes.").unwrap();
    writeln!(
        errors_file,
        "// This file is autogenerated, do not modify!. See `generate_tags.rs`"
    )
    ...
    for error in errors {
        ...
        let error_num: u16 = error
            .get("error")
            .expect("Each error must have an error number.")
            .as_number()
...
back_to_enum! {
    ...
    /// [`crate::labjack::all_tags::LAST_ERR_DETAIL`] tag.
    #[derive(Debug, Error)]
    #[repr(u16)]
